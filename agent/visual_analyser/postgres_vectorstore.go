...
func (s *PostgresVectorStore) Upsert(ctx context.Context, req *base.UpsertRequest) error {
	// Validate input
	if req.AssetID == "" {
	...
	metadataJSON, err := base.MetadataToJSON(req.Metadata)
	if err != nil {
...
func (s *PostgresVectorStore) SearchNearest(ctx context.Context, req *base.SearchRequest) (*base.SearchResponse, error) {
	// Validate input
	if len(req.Embedding) == 0 {
	...
	var results []base.SimilarAsset
	for rows.Next() {
		var asset base.SimilarAsset
		...
		// Parse metadata
		asset.Metadata, err = base.JSONToMetadata(metadataJSON)
		if err != nil {
...
func (s *PostgresVectorStore) GetByAssetID(ctx context.Context, assetID string) (*base.IPAsset, error) {
	query := `
...
func (s *PostgresVectorStore) GetByOwnerID(ctx context.Context, ownerID string) ([]*base.IPAsset, error) {
	query := `
		SELECT id, asset_id, owner_id, embedding, metadata, created_at, updated_at
		FROM ip_asset_embeddings
		WHERE owner_id = $1
